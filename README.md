...
## Discover Music Through Lyrics, From Anywhere in the World
...
- **Genre Filter**: Filter by genre tags like Afrobeat, Hip Hop, R&B, Pop, Reggae, and more
- **Smart Result Ranking**: Results respect Genius's relevance ordering, so specific lyric phrases surface exact matches first
- **Artist Enrichment**: Country of origin and genre tags are loaded automatically from MusicBrainz, a free community-driven database
...
3. Results come back in relevance order, so specific phrases match their exact songs first
...
- **Genius API** is free with no hard rate limit for reasonable usage. It handles all the full-text lyrics searching.
- **MusicBrainz API** is completely free and needs no API key. It's rate-limited to 1 request/second (enforced server-side). Artist metadata is cached in memory, so repeated searches are fast.
- **Parallel execution** lets filtered searches run multiple Genius queries concurrently using Python's `ThreadPoolExecutor` to minimize latency.
- **Caching** with in-memory stores for MusicBrainz artist lookups and country artist lists reduces redundant API calls and speeds up repeat searches.
